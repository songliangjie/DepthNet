...
class ChannelAttentionFusion(nn.Module):
    """
    Channel attention fusion for VIS and IR depth features.

    The module predicts per-channel modality weights from global descriptors,
    then refines the weighted feature with ConvGNAct to match the depth branch
    style.
    """
    def __init__(self, channels: int, reduction: int = 4):
        ...
        self.refine = ConvGNAct(channels, channels)

    def forward(self, vis_feat: torch.Tensor, ir_feat: torch.Tensor) -> torch.Tensor:
        if vis_feat.shape != ir_feat.shape:
            raise ValueError(
                f"VIS and IR feature shapes must match, got {vis_feat.shape} and {ir_feat.shape}"
            )

        b, c, _, _ = vis_feat.shape
        weights = self.weight_gen(torch.cat([vis_feat, ir_feat], dim=1))
        weights = weights.view(b, 2, c, 1, 1).softmax(dim=1)
...
class DepthBranchF(nn.Module):
    ...
    架构：
        - 编码器（自下而上）：VIS/IR双流5层卷积，逐层通道注意力融合
        - 金字塔结构（自上而下）：横向连接 + 上采样
    ...
    def __init__(self, in_ch=1, chs=(16, 32, 64, 128, 256), regress_depth=True):
        ...
        # VIS encoder stream (5 layers) - bottom-up pathway
        self.encoder_vis = nn.ModuleDict({
            "enc1": nn.Sequential(ConvGNAct(in_ch, c1), ConvGNAct(c1, c1)),
            "down1": DepthDown(c1),
            "enc2": nn.Sequential(ConvGNAct(c1, c2), ConvGNAct(c2, c2)),
            "down2": DepthDown(c2),
            "enc3": nn.Sequential(ConvGNAct(c2, c3), ConvGNAct(c3, c3)),
            "down3": DepthDown(c3),
            "enc4": nn.Sequential(ConvGNAct(c3, c4), ConvGNAct(c4, c4)),
            "down4": DepthDown(c4),
            "enc5": nn.Sequential(ConvGNAct(c4, c5), ConvGNAct(c5, c5)),
        })

        # IR encoder stream mirrors VIS so each scale keeps channels c1~c5.
        self.encoder_ir = nn.ModuleDict({
            "enc1": nn.Sequential(ConvGNAct(in_ch, c1), ConvGNAct(c1, c1)),
            "down1": DepthDown(c1),
            "enc2": nn.Sequential(ConvGNAct(c1, c2), ConvGNAct(c2, c2)),
            "down2": DepthDown(c2),
        ...
        self.fusion_layers = nn.ModuleList([
            ChannelAttentionFusion(c1),
            ChannelAttentionFusion(c2),
            ChannelAttentionFusion(c3),
            ChannelAttentionFusion(c4),
            ChannelAttentionFusion(c5),
        ])
    ...
    def forward_encoder(self, vis: torch.Tensor, ir: torch.Tensor) -> List[torch.Tensor]:
        """
        ...
        """
        vis1 = self.encoder_vis["enc1"](vis)
        ir1 = self.encoder_ir["enc1"](ir)
        f1 = self.fusion_layers[0](vis1, ir1)

        vis2_in = self.encoder_vis["down1"](f1)
        ir2_in = self.encoder_ir["down1"](f1)
        vis2 = self.encoder_vis["enc2"](vis2_in)
        ir2 = self.encoder_ir["enc2"](ir2_in)
        f2 = self.fusion_layers[1](vis2, ir2)

        vis3_in = self.encoder_vis["down2"](f2)
        ir3_in = self.encoder_ir["down2"](f2)
        vis3 = self.encoder_vis["enc3"](vis3_in)
        ir3 = self.encoder_ir["enc3"](ir3_in)
        f3 = self.fusion_layers[2](vis3, ir3)

        vis4_in = self.encoder_vis["down3"](f3)
        ir4_in = self.encoder_ir["down3"](f3)
        vis4 = self.encoder_vis["enc4"](vis4_in)
        ir4 = self.encoder_ir["enc4"](ir4_in)
        f4 = self.fusion_layers[3](vis4, ir4)

        vis5_in = self.encoder_vis["down4"](f4)
        ir5_in = self.encoder_ir["down4"](f4)
        vis5 = self.encoder_vis["enc5"](vis5_in)
        ir5 = self.encoder_ir["enc5"](ir5_in)
        f5 = self.fusion_layers[4](vis5, ir5)
        return [f1, f2, f3, f4, f5]
    
    def forward(self, vis: torch.Tensor, ir: torch.Tensor) -> Tuple[Optional[torch.Tensor], List[torch.Tensor]]:
        """
        ...
        """
        # Bottom-up pathway (dual-stream collaborative encoder)
        dep_feats = self.forward_encoder(vis, ir)
        f1, f2, f3, f4, f5 = dep_feats
